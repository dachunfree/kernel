...
static int fs_recent_filesize = 0;
int fs_get_resent_filesize()
{
	char *env;
	char temp[32] = {0};
	int file_len = 0;

	env = getenv("bitfilesize");
	if(NULL == env)
	{
		return 0;
	}
	...
	fs_recent_filesize = file_len;
	return fs_recent_filesize;
}
		...
		int fstype)
{
	...
	if((BIT_LOADER_ADDR == addr) || (0 == strncmp(filename,"cap_mac_fpga.bit", strlen(filename))))
	{
		setenv_hex("bitfilesize", len_read);
	}
	return 0;
}
