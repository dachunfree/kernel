...
static int do_env_read_from_cur_dev(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int i;
	int rcode = 0;
	int env_flag = H_HIDE_DOT;
...
static cmd_tbl_t cmd_env_sub[] = {
	...
	U_BOOT_CMD_MKENT(getenv, CONFIG_SYS_MAXARGS, 1, do_env_read_from_cur_dev, "", ""),
...
static char env_help_text[] =
	...
	"env getenv [-a | name ...] - print  cur dev  environment\n"
#if defined(CONFIG_CMD_RUN)
...
U_BOOT_CMD_COMPLETE(
	getenv, CONFIG_SYS_MAXARGS, 1,  do_env_read_from_cur_dev,
	"update environment variables from current device",
	"[-a]\n    - print [all] values of all environment variables\n"
	"upenv name ...\n"
	"    - print value of environment variable 'name'",
	var_complete
