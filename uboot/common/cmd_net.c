...
int do_tftpb(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if(ZYNQ_TFTPBOOT_FILE_FLAG == tftp_file_flag)/*tftpboot file is not existent*/
	{
		return ZYNQ_TFTPBOOT_FILE_FLAG;
	}
		...
		char * const argv[])
{
	...
	if((BIT_LOADER_ADDR == load_addr) || (0 == strncmp(net_boot_file_name,"cap_mac_fpga.bit", strlen(net_boot_file_name))))
	{
		setenv_hex("bitfilesize", size);
	}
