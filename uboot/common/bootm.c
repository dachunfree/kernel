...
/*
 *  transmit additional bootargs to kernel,for checking uboot version in kernel
**/
static int add_version2args(void)
{
	...
	debug("CONFIG_LOCALVERSION is:%s \n",CONFIG_LOCALVERSION);
	...
	if (hasadded) {
		...
		if (args != NULL) {
			strcpy(newargs, "ver=");
			strcat(newargs, CONFIG_LOCALVERSION);
			strcat(newargs, " ");
			strcat(newargs, args);
