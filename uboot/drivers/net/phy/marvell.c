...
static int m88e1512_read_status(struct phy_device *phydev)
{
	...
	int lpa;
	int status = 0;
	int oldpage = phy_read(phydev, MDIO_DEVAD_NONE,22);
	// select copper
	phy_write(phydev, MDIO_DEVAD_NONE, 22, 0);
	status = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_88E1xxx_PHY_STATUS);
	if (status < 0) {
		phy_write(phydev, MDIO_DEVAD_NONE, 22, oldpage);  // restore page
		return status;
	}

	if (!(status & MIIM_88E1xxx_PHYSTAT_LINK)) {
		// if copper link down, select fiber
		phy_write(phydev, MDIO_DEVAD_NONE, 22, 1);
		status = phy_read(phydev, MDIO_DEVAD_NONE, MIIM_88E1xxx_PHY_STATUS);
		if (status < 0) {
			phy_write(phydev, MDIO_DEVAD_NONE, 22, oldpage);  // restore page
			printf("fiber link error\n");
			return status;
		}
	}
	...
	if (AUTONEG_ENABLE == phydev->autoneg) {
		...
		if (status & MIIM_88E1xxx_PHYSTAT_DUPLEX)
			phydev->duplex = DUPLEX_FULL;
		else
			phydev->duplex = DUPLEX_HALF;

		status = status & MIIM_88E1xxx_PHYSTAT_SPEED;
