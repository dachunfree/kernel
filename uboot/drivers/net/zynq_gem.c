...
#define PHY_DETECT_REG_ID_H	2
#define PHY_DETECT_REG_ID_L	3


#define PHY_DETECT_REG_NUM0	0
#define PHY_ADDR		0
 ...
 */
#define PHY_DETECT_MASK 0x2000a231

static int phyid_mask = PHY_DETECT_MASK;
static int phyid_delay = 100;
/* TX BD status masks */
...
static u32 get_phyid(struct zynq_gem_priv *priv,int i)
{
	u16 reg0,reg1;
	u32 phyid;
	phyread(priv, i, PHY_DETECT_REG_ID_H, &reg0);
	if ((reg0 & 0xffff) == 0xffff ) {
		return 0xffffffff;
	}

	phyread(priv, i, PHY_DETECT_REG_ID_L, &reg1);
	if ((reg0 & 0xffff) == 0xffff ){
		return 0xffffffff;
	}
...
static int phy_detection(struct udevice *dev)
{
	...
	printf("entern %s: phyaddr = %x phyid_mask:%x delay:%d ms\n",
		__func__,priv->phyaddr, phyid_mask, phyid_delay);
	udelay(1000 * phyid_delay);
	if (priv->phyaddr != -1) {
		phyreg = get_phyid(priv, priv->phyaddr);
		if ((phyreg != 0xFFFFFFFF) &&
		    ((phyreg & phyid_mask) == phyid_mask)) {
			/* Found a valid PHY address */
			debug("Default phy address %d is valid\n",
			      priv->phyaddr);
			printf("Found valid phy address: %08x\n", phyreg);
			return 0;
		} else {
		...
		if (phyreg != 0xFFFFFFFF)
			printf("Found phyid: %08x\n", phyreg);
	}

	debug("detectingq phy address\n");
	if (priv->phyaddr == -1) {
		/* detect the PHY address */
		for (i = 31; i >= 0; i--) {
			phyreg = get_phyid(priv, i);
			if ((phyreg != 0xFFFFFFFF) &&
			    ((phyreg & phyid_mask) == phyid_mask)) {
				/* Found a valid PHY address */
				priv->phyaddr = i;
				debug("Found valid phy address, %d\n", i);
				printf("Found valid phy address, %d: %08x\n", i, phyreg);
				return 0;
			}
			if (phyreg != 0xFFFFFFFF)
				printf("Found phyid: %08x\n", phyreg);
...
static int zynq_gem_init(struct udevice *dev)
{
	...
	if(NULL == priv->phydev)
	{
		printf("Warning: phy is not ready!\n");
		return -1;
	}
...
static int zynq_gem_probe(struct udevice *dev)
{
	...
	if (ret)
		return ret;
#if 0
	phyread(priv,PHY_ADDR,PHY_DETECT_REG_NUM0,&temp);
	printf("the phy read reg 0x0 value = 0x%x\n",temp);
	temp = temp&(~0x8000);
	phywrite(priv,PHY_ADDR,PHY_DETECT_REG_NUM0,&temp);
	int i = 0;
	for(i; i < 10; i++) {
		phyread(priv,PHY_ADDR,PHY_DETECT_REG_NUM0,&temp);
		if (temp & 0x8000) {
...
static int zynq_gem_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (offset > 0) {
		...
		phyid_mask = fdtdec_get_int(gd->fdt_blob, offset, "phyid_mask", PHY_DETECT_MASK);
		phyid_delay = fdtdec_get_int(gd->fdt_blob, offset, "phyid_delay", 100);
	}
