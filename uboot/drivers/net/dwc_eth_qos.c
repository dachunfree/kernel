...
static int eqos_start(struct udevice *dev)
{
	...
	if(eqos->interface == PHY_INTERFACE_MODE_SGMII)
		eqos_mdio_write(eqos->mii, 0x0, MDIO_DEVAD_NONE, 0x0, 0x0140);
...
static int eqos_probe(struct udevice *dev)
{
	...
	zynq_slcr_unlock();
	writel(0x3, FPGA_RST_CTRL);
	udelay(10000);
	writel(0x7, FPGA_RST_CTRL);
	udelay(10000);
	writel(0x3, FPGA_RST_CTRL);
	udelay(10000);
	writel(0x0, FPGA_RST_CTRL);
	udelay(10000);
	zynq_slcr_lock();
	...
	eqos->phy = phy_connect(eqos->mii, eqos->phyaddr, dev, eqos->interface);
	if (!eqos->phy) {
...
static int eqos_ofdata_to_platdata(struct udevice *dev)
{
	...
	offset = fdtdec_lookup_phandle(gd->fdt_blob, dev->of_offset,
				       "phy-handle");
	if (offset > 0) {
		eqos->phyaddr = fdtdec_get_int(gd->fdt_blob, offset, "reg", -1);
	}

	phy_mode = fdt_getprop(gd->fdt_blob, dev->of_offset, "phy-mode", NULL);
	if (phy_mode)
		plat->phy_interface = phy_get_interface_by_name(phy_mode);
	if (plat->phy_interface == -1) {
		debug("%s: Invalid PHY interface '%s'\n", __func__, phy_mode);
		return -EINVAL;
	}
	eqos->interface = plat->phy_interface;

	printf("dwc_eth_qos: phyaddr %d, interface %s\n",
	       eqos->phyaddr, phy_string_for_interface(eqos->interface));
