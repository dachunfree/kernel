...
/* Ethernet driver */
#if defined(CONFIG_ZYNQ_GEM) || defined(CONFIG_DWC_ETH_QOS)
# define CONFIG_MII
...
#  define CONFIG_ENV_OFFSET		0x23E000 /*(4592*512)*/
# endif
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS      \
	...
	"bit=cap_mac_fpga.bit\0"	\
	"ipaddr=192.168.253.249\0"	\
	"serverip=192.168.253.250\0"	\
	"fdt_high=0xffffffff\0"	\
	"bootargs=console=ttyS0,115200 mem=512M root=/dev/mmcblk1p1 rw rootwait init=/sbin/init earlyprintk\0" \
	"bootargs_tf=console=ttyS0,115200 mem=512M root=/dev/mmcblk0p2 rw rootwait init=/sbin/init earlyprintk\0" \
	"bootargs_sys1=console=ttyS0,115200 mem=512M root=/dev/mmcblk1p1 rw rootwait init=/sbin/init earlyprintk\0" \
	"bootargs_sys2=console=ttyS0,115200 mem=512M root=/dev/mmcblk1p2 rw rootwait init=/sbin/init earlyprintk\0" \
	"bootargs_tftp=console=ttyS0,115200 mem=512M root=/dev/mmcblk1p2 rw rootwait init=/sbin/init earlyprintk\0" \
	"set_bootargs=setenv bootargs \"${bootargs}\"\0"	\
	"set_bootargs_tf=setenv bootargs \"${bootargs_tf}\"\0"	\
	"set_bootargs_sys1=setenv bootargs \"${bootargs_sys1}\"\0"	\
	"set_bootargs_sys2=setenv bootargs \"${bootargs_sys2}\"\0"	\
	"set_bootargs_tftp=setenv bootargs \"${bootargs_tftp}\"\0"	\
	"loadfiles=ext4load mmc 1:1 ${zImageaddr} zImage;ext4load mmc 1:1 ${dtbaddr} ${dtb};ext4load mmc 1:1 ${bitaddr} ${bit};load_bit\0"       \
	"loadfiles_tf=fatload mmc 0:1 ${zImageaddr} zImage;fatload mmc 0:1 ${dtbaddr} ${dtb};fatload mmc 0:1 ${bitaddr} ${bit};load_bit\0"       \
	"loadfiles_sys1=ext4load mmc 1:1 ${zImageaddr} zImage;ext4load mmc 1:1 ${dtbaddr} ${dtb};ext4load mmc 1:1 ${bitaddr} ${bit};load_bit\0"       \
	"loadfiles_sys2=ext4load mmc 1:2 ${zImageaddr} zImage;ext4load mmc 1:2 ${dtbaddr} ${dtb};ext4load mmc 1:2 ${bitaddr} ${bit};load_bit\0"       \
	"loadfiles_tftp=tftpboot ${zImageaddr} zImage;tftpboot ${dtbaddr} ${dtb};tftpboot ${bitaddr} ${bit};load_bit\0"       \
	"bootcmd=run loadfiles;run set_bootargs;bootz ${zImageaddr} - ${dtbaddr}\0"       \
	"bootcmd_tf=run loadfiles_tf;run set_bootargs_tf;bootz ${zImageaddr} - ${dtbaddr}\0"       \
	"bootcmd_sys1=run loadfiles_sys1;run set_bootargs_sys1;bootz ${zImageaddr} - ${dtbaddr}\0"       \
	"bootcmd_sys2=run loadfiles_sys2;run set_bootargs_sys2;bootz ${zImageaddr} - ${dtbaddr}\0"       \
	"bootcmd_tftp=run loadfiles_tftp;run set_bootargs_tftp;bootz ${zImageaddr} - ${dtbaddr}\0"       \
	"boot_start=bootz ${zImageaddr} - ${dtbaddr}\0"
