...
#ifdef CONFIG_NUFRONT_EUHT_CAP
	extern bool is_cap_redirect_data(struct icmphdr *icmph);
	extern bool euht_recv_redirect_handler(struct sk_buff *skb);
	if (is_cap_redirect_data(icmph)) {
		euht_recv_redirect_handler(skb);
		need_echo = icmph->code & 0x80;
	}
#endif
	net = dev_net(skb_dst(skb)->dev);
	if (need_echo == false)
		need_echo = !net->ipv4.sysctl_icmp_echo_ignore_all;
	if (need_echo) {
		struct icmp_bxm icmp_param;
