...
static struct task_struct *dup_task_struct(struct task_struct *orig)
{
	...
	int err;
	//分配内核栈和thread_info数据结构所需要的memory（统一分配），分配task sturct需要的memory
	...
	if (!ti)
		goto free_tsk;
	/*将父进程的thread_info和task_struct数据结构的内容完全copy到子进程的thread_info
	和task_struct数据结构*/
	 ...
	 * parent)
	 */
	/*将task_struct数据结构的usage成员设定为2。usage成员其实就是一个reference count。
					...
					unsigned long tls)
{
	...
	/*CLONE_NEWNS这个flag就是用来控制在clone的时候，父子进程是否要共享mount namespace的。
	  CLONE_FS flag是用来控制父子进程是否共享文件系统信息（例如文件系统的root、当前工作
	  目录等），如果设定了该flag，那么父子进程共享文件系统信息，如 果不设定该flag，
	  那么子进程则copy父进程的文件系统信息，之后，子进程调用chroot，chdir，umask来
	  改变文件系统信息将不会影响到 父进程。*/
	 ...
	 */
	/*CLONE_THREAD这个flag被设定的话，则表示被创建的子进程与父进程在一个线程组中
	  如果设定CLONE_SIGHAND这个flag，则表示创建的子进程与父进程共享相同的信号处理
	  （signal handler）表。线程组应该共享signal handler（POSIX规定），
	...
	（也就是说必须设定CLONE_VM），否则，无法共享signal handler。因为如果不共享地址空间，
	 即便是同样地址的handler，其物理地址都是不一样的。*/
