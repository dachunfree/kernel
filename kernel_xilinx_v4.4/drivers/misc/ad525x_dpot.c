...
static s32 dpot_read_i2c(struct dpot_data *dpot, u32 reg)
{
	int value;
	unsigned ctrl = 0;
	struct i2c_client *client = (struct i2c_client *)dpot->bdata.client;
	switch (dpot->uid) {
	case DPOT_UID(AD5122_ID):
		{
			//printk(KERN_ERR"reg = 0x%x \n\r",reg);
			if(reg & DPOT_ADDR_EEPROM) {
				//printk(KERN_ERR"this is eeprom \n\r");
				if(DPOT_RDAC0 == (reg & 0xf)) {
					ctrl = AD5122_EEPROM0;
				}else if(DPOT_RDAC1 == (reg & 0xf)){
					ctrl = AD5122_EEPROM1;
				}
			}else if(reg & DPOT_ADDR_SHIFT) {
				//printk(KERN_ERR"this is shitf \n\r");
				if(DPOT_RDAC0 == (reg & 0xf)) {
					ctrl = AD5122_SHIFT0;
				}else if(DPOT_RDAC1 == (reg & 0xf)){
					ctrl = AD5122_SHIFT1;
			...
			//printk(KERN_ERR"read:device:0x%x,adr:0x%x,\n\r",client->addr,ctrl);
		 	return dpot_read_r8d8(dpot,ctrl);
		}
...
static s32 dpot_read(struct dpot_data *dpot, u32 reg)
{
	if (dpot->feat & F_SPI)
...
static s32 dpot_write_i2c(struct dpot_data *dpot, u32 reg, u16 value)
{
	...
	case DPOT_UID(AD5122_ID):
		{
			...
			//printk(KERN_ERR"device:0x%x,adr:0x%x,value = %d \n\r",client->addr,ctrl,value);
...
static s32 dpot_write(struct dpot_data *dpot, u32 reg, u16 value)
{
	if (dpot->feat & F_SPI)
...
DPOT_DEVICE_SHOW_SET(shift0, DPOT_ADDR_SHIFT | DPOT_RDAC0);
...
DPOT_DEVICE_SHOW_SET(shift1, DPOT_ADDR_SHIFT | DPOT_RDAC0);
...
DPOT_DEVICE_SHOW_SET(shift2, DPOT_ADDR_SHIFT | DPOT_RDAC0);
...
DPOT_DEVICE_SHOW_SET(shift3, DPOT_ADDR_SHIFT | DPOT_RDAC0);
...
DPOT_DEVICE_SHOW_SET(shift4, DPOT_ADDR_SHIFT | DPOT_RDAC0);
...
DPOT_DEVICE_SHOW_SET(shift5, DPOT_ADDR_SHIFT | DPOT_RDAC0);
...
static const struct attribute *dpot_attrib_shift[] = {
	&dev_attr_shift0.attr,
	&dev_attr_shift1.attr,
	&dev_attr_shift2.attr,
	&dev_attr_shift3.attr,
	&dev_attr_shift4.attr,
	&dev_attr_shift5.attr,
	NULL
};
		...
		unsigned features, unsigned rdac)
{
	...
	if (features & F_CMD_SHIFT)
		err |= sysfs_create_file(&dev->kobj,
			dpot_attrib_shift[rdac]);
