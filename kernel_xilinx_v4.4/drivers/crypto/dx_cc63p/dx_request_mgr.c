		...
		unsigned int total_seq_len)
{
	...
	if ((likely(req_mgr_h->q_free_slots >= total_seq_len)) ) {
		DX_LOG_DEBUG("HW FIFO info. req_queue_head=%d "
	...
	HwDesc_s *desc, unsigned int len, bool is_dout)
{
...
#if 0
	u32 q_free_slots_s = READ_REGISTER(cc_base + (
				DX_CC_REG_OFFSET(CRY_KERNEL,
						 DSCRPTR_QUEUE0_CONTENT)));
	DX_LOG_DEBUG("hw 2 q q_free_slots = %d.\n", q_free_slots_s);
#endif
	...
	if (!is_dout) {
		...
		*  Return "0" -Operation done successfully. */
		rc = wait_for_completion_interruptible(&dx_req->seq_compl);
		return rc;
	} else {
