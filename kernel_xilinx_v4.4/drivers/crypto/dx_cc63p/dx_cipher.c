				 ...
				 enum sep_crypto_direction direction)
{
	...
	DECL_CYCLE_COUNT_RESOURCES;
	#if 0
	DX_LOG_DEBUG("sw q q_free_slots = %d.\n", req_mgr_h->q_free_slots);

	u32 q_free_slots_s = READ_REGISTER(ctx_p->drvdata->cc_base + (
				DX_CC_REG_OFFSET(CRY_KERNEL,
						 DSCRPTR_QUEUE0_CONTENT)));
	DX_LOG_DEBUG("hw q q_free_slots = %d.\n", q_free_slots_s);
	#endif
...
int dx_ablkcipher_alloc(struct dx_drvdata *drvdata)
{
	...
	WRITE_REGISTER(drvdata->cc_base +
		       ...
		       (ablkcipher_handle->dropped_buffer_dma_addr >> MAX_SHIFGT));
#endif
