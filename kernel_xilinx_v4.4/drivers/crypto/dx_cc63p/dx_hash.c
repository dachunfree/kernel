...
int dx_hash_alloc(struct dx_drvdata *drvdata)
{
	...
	for (alg = 0; alg < ARRAY_SIZE(driver_hash); alg++) {
		...
		if ((((struct dx_hash_template)driver_hash[alg]).hw_mode != SEP_CIPHER_XCBC_MAC) &&
			...
			if (IS_ERR(t_alg)) {
				rc = PTR_ERR(t_alg);
				DX_LOG_ERR("%s hmac alg allocation failed\n",
			...
			if (unlikely(rc != 0)) {
				DX_LOG_ERR("%s hmac alg registration failed\n",
		...
		if (IS_ERR(t_alg)) {
			rc = PTR_ERR(t_alg);
			DX_LOG_ERR("%s hash alg allocation failed\n",
				 driver_hash[alg].driver_name);
			goto fail;
		}
		t_alg->drvdata = drvdata;
		...
		rc = crypto_register_ahash(&t_alg->ahash_alg);
		if (unlikely(rc != 0)) {
			DX_LOG_ERR("%s hash alg registration failed\n",
				t_alg->ahash_alg.halg.base.cra_driver_name);
			kfree(t_alg);
			goto fail;
		} else
