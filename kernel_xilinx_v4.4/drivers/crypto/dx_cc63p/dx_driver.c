...
static int cc63_probe(struct platform_device *plat_dev)
{
	...
	unsigned int val;
	//struct clk *clk;
	void __iomem *addr_ddr;
#if defined(CONFIG_ARM) && defined(DX_DEBUG)
	uint32_t ctr, cacheline_size;
#endif

	/* ddr shoule set non secure mode in ZYNQ */
	printk(KERN_INFO "%s\n",__func__);
	if (!request_mem_region(0xf8000430, 0x10, "ddr reg"))
		return -1;

	/* TODO: Do not use magic number, and add a commen for this code */
	addr_ddr = ioremap(0xf8000430, 0x10);
	writel(0xffffffff, addr_ddr);
	val = readl(addr_ddr);
	printk(KERN_INFO "ddr non secure mode val= 0x%x\n", val);
	iounmap(addr_ddr);
...
#endif
	//DX_LOG_DEBUG("before setting, ccp clk register:  0x%08x\n", nufront_prcm_readl(0x20C));

#if 0
	clk = devm_clk_get(&plat_dev->dev, "ccp_aclk");
	if (!IS_ERR(clk))
		clk_prepare_enable(clk);

	clk = devm_clk_get(&plat_dev->dev, "ccp_pclk");
	if (!IS_ERR(clk))
		clk_prepare_enable(clk);
#endif
	//DX_LOG_DEBUG("after setting, ccp clk register:  0x%08x\n", nufront_prcm_readl(0x20C));
