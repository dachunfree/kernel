...
#include <linux/tcp.h>
#include <linux/skb_pool.h>
#include "dwc.h"
#include "dwc_eth_ptp.h"
#include <soc/nufront/cap_main.h>
#include <soc/nufront/euht_ex.h>
...
#define DWCEQOS_RX_DCNT 1024//512//256
#define DWCEQOS_TX_DCNT 1024///256

#define DWCEQOS_HASH_TABLE_SIZE 64
#define DWCEQOS_ETH_HEAD_SIZE 32
...
#define REG_DWCEQOS_DMA_CHO_RX_INT_WDOG  0x1138
...
#define DWCEQOS_DMA_CH0_IE_NIE           BIT(16)
#define DWCEQOS_DMA_CH0_IE_AIE           BIT(15)
...
#define DWCEQOS_DMA_CH0_IE_TBUE		 BIT(2)
...
#define REG_DWCEQOS_MAC_RXQ0_CTRL2       0x00a8
...
#define LED_6
unsigned int *vir_led6 = NULL;

struct skb_pool *dwc_skb_pool = NULL;


static int debug = 3;
module_param(debug, int, 0);
MODULE_PARM_DESC(debug, "DWC_eth_qos debug level (0=none,...,16=all)");
#ifdef HOLD_ARP_FOR_EUHT
static u16 arp_holdtime = 0;
module_param_named(hoarp_holdtime, arp_holdtime, uint, (S_IRUGO | S_IWUSR | S_IWGRP));
MODULE_PARM_DESC(hoarp_holdtime, "Delayed with updating mac-entries to prevent packet loss after handover.");

struct euht_hoarp_ops {
...
static void arp_delay_timer_expired(unsigned long data);
static struct timer_list arp_delay_timer =
		TIMER_INITIALIZER(arp_delay_timer_expired,
								0, 0);

static void arp_delay_timer_expired(unsigned long data)
{
	unsigned int expire_time;
	struct euht_hoarp_ops *hoarp_ops, *tmp;

	if (arp_holdtime == 0 || list_empty(&arp_list) == 1) {
		mod_timer(&arp_delay_timer, jiffies + 3);
		return;
	}

	list_for_each_entry_safe(hoarp_ops, tmp, &arp_list, list) {
		hoarp_ops->ageing_time -= 30;
		if ((hoarp_ops->ageing_time) <= 0 ) {
			printk(KERN_DEBUG "##%s jiffies: %lu\n", __func__, jiffies);
			netif_receive_skb(hoarp_ops->delay_skb);
			list_del(&hoarp_ops->list);
			kfree(hoarp_ops);
		}
	}
	mod_timer(&arp_delay_timer, jiffies + 3);
...
extern struct euht_macaddr_ops macaddr_ops;
#endif
...
static const struct dwceqos_stat dwceqos_ethtool_stats[] = {
	...
	/*ptp*/
	STAT_ITEM("ptp_ver", ptp_ver),
	STAT_ITEM("ptp_frame_type", ptp_frame_type),
	STAT_ITEM("rx_msg_type_sync", rx_msg_type_sync),
	STAT_ITEM("rx_msg_type_ext_no_ptp", rx_msg_type_ext_no_ptp),
	STAT_ITEM("rx_msg_type_follow_up", rx_msg_type_follow_up),
	STAT_ITEM("rx_msg_type_delay_req", rx_msg_type_delay_req),
	STAT_ITEM("rx_msg_type_delay_resp", rx_msg_type_delay_resp),
	STAT_ITEM("rx_msg_type_pdelay_req", rx_msg_type_pdelay_req),
	STAT_ITEM("rx_msg_type_pdelay_resp", rx_msg_type_pdelay_resp),
	STAT_ITEM("rx_msg_type_pdelay_follow_up", rx_msg_type_pdelay_follow_up),
	STAT_ITEM("timestamp_dropped", timestamp_dropped),
};
...
static void print_status(struct net_local *lp)
{
	...
	pr_info("DMA_interrupt_ch0_status: 0x%08x\n",
		dwceqos_read(lp, 0x1160));
	pr_info("DMA_interrupt_status: 0x%08x\n",
		dwceqos_read(lp, 0x1008));
	pr_info("DMA_interrupt_Enable: 0x%08x\n",
...
#if CONFIG_SYNOPSYS_DWC_PTP
	case SIOCSHWTSTAMP:
		return dwc_ptp_ioctl(ndev, rq);
#endif
...
static void dwceqos_alloc_rxring_desc(struct net_local *lp, int index)
{
	...
	if (lp->support_skb_pool){

		struct skb_entry *skbe = skb_entry_get(dwc_skb_pool,GFP_ATOMIC);
		if(!skbe){
			netdev_err(lp->ndev,"%s :    can.t  allocate the skb entry\n",__func__);
			goto err_out;
		}
		new_skb = skbe->skb;
		new_skb_baddr = skbe->dma_map_start;
		skbe->state = USING_ETH;
	}else{
		new_skb = netdev_alloc_skb(lp->ndev, DWCEQOS_RX_BUF_SIZE + 64);
		if (!new_skb) {
...
static irqreturn_t dwceqos_interrupt(int irq, void *dev_id)
{
	...
	if (cause & DWCEQOS_DMA_IS_DC0IS) {
		...
		if (dma_status & DWCEQOS_DMA_CH0_IS_RI) {
			...
			recv_packet++;
			if(vir_led6 != NULL) {			
				unsigned int tmp = recv_packet % 20;
				if(tmp >=0 && tmp < 10) {
					*vir_led6 = 0x0000ffff;
				}else {
					*vir_led6 = 0xffff0000;
...
static void dwceqos_tx_reclaim(unsigned long data)
{
	...
	while (lp->tx_free < DWCEQOS_TX_DCNT) {
		...
		if (dd->des3 & DWCEQOS_DMA_TDES3_OWN)
			break;
		if(rd->mapping && !rd->is_forward)
			dma_unmap_single(ndev->dev.parent, rd->mapping, rd->len,
					 DMA_TO_DEVICE);

#if CONFIG_SYNOPSYS_DWC_PTP
		if (lp->hwts_tx_en) {
			dwc_ptp_tx_hwtstamp(lp, dd, rd->skb);
		}
#endif
...
static int dwceqos_rx(struct net_local *lp, int budget)
{
	...
	while (n_descs < budget) {
		...
		if(lp->support_skb_pool){
			struct skb_entry *skbe = skb_entry_get(dwc_skb_pool,GFP_ATOMIC);
			if(!skbe){
				netdev_err(lp->ndev,"%s : get skb entry failed\n",__func__);
				break;
			}
			skbe->state = USING_ETH;
			new_skb = skbe->skb;
		...
		if(!lp->support_skb_pool){
		/* Unmap old buffer */
			if (dma_map_optimized)
				dma_unmap_single(lp->ndev->dev.parent,
					 lp->rx_skb[lp->rx_cur].mapping,
					 DWCEQOS_ETH_HEAD_SIZE, DMA_FROM_DEVICE);
			else
				dma_unmap_single(lp->ndev->dev.parent,
					lp->rx_skb[lp->rx_cur].mapping,
		...
		if ((dd->des3 & DWCEQOS_DMA_RDES3_ES) ||
...
#if CONFIG_SYNOPSYS_DWC_PTP
			if (lp->hwts_rx_en) {
				dwc_ptp_rx_hwtstamp(lp,
							dd,
							&lp->rx_descs[(lp->rx_cur + 1) % DWCEQOS_RX_DCNT],
							skb);
			}
#endif
			skb_put(skb, len);
			...
			if(lp->support_skb_pool){
				packet_type = skb_eth_packet_judge(skb , lp->ndev);
				if(packet_type != PACKET_OTHERHOST){
					nskb = alloc_skb(skb->len + 2,GFP_ATOMIC);
					if(!nskb){
						netdev_err(lp->ndev,"%s : can.t alloc new skb\n",__func__);
						dev_kfree_skb_any(skb);
						break;
					}
					skb_reserve(nskb, 2);
...
#ifdef HOLD_ARP_FOR_EUHT
			/* euht ho arp hold*/
			if (arp_holdtime >= 10 && arp_holdtime < 65536
					&& get_protocol(skb) == htons(ETH_P_ARP) && macaddr_ops.mac_addr_find != NULL) {
					struct ethhdr *ethhdr = eth_hdr(skb);
					if (macaddr_ops.mac_addr_find(ethhdr->h_source) != 0xffe) {
						struct euht_hoarp_ops *hoarp_ops;
						hoarp_ops = kzalloc(sizeof(struct euht_hoarp_ops), GFP_KERNEL);
						printk(KERN_DEBUG "##%s holdtime: %d jiffies: %lu\n", __func__, arp_holdtime, jiffies);
						INIT_LIST_HEAD(&hoarp_ops->list);
						hoarp_ops->delay_skb = skb;
						hoarp_ops->ageing_time = arp_holdtime;
						list_add_tail(&hoarp_ops->list, &arp_list);
					} else
...
static int dwceqos_open(struct net_device *ndev)
{
	 ...
	 */
	if(lp->phy_interface == PHY_INTERFACE_MODE_SGMII)
		dwceqos_mdio_write(lp->mii_bus, 0x0, 0x0, 0x0140);
			     ...
			     struct dwceqos_tx *tx)
{
	...
	dma_addr_t dma_addr_offset;

	if(skb_is_forward(skb)){
		is_forward = true;
		skbe = (struct skb_entry *)(*(unsigned int *)skb->head);
		dma_addr_offset = skbe->map + skb_headroom(skb);
	}else
		is_forward = false;
	...
	if(is_forward)
		dma_handle = dma_addr_offset;
	else
		dma_handle = dma_map_single(lp->ndev->dev.parent, skb->data,
				    skb_headlen(skb), DMA_TO_DEVICE);

	if (!is_forward && dma_mapping_error(lp->ndev->dev.parent, dma_handle)) {
...
static int dwceqos_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	if (unlikely((skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP) &&
			lp->hwts_tx_en)) {
		dwc_ptp_enable_tx(&lp->tx_descs[trans.last_descriptor], skb);
	}
#endif
...
static int dwceqos_probe(struct platform_device *pdev)
{
...
#endif
#ifdef LED_6
	vir_led6 = ioremap(0x40000284,4);
	if(vir_led6 == NULL) {
		printk("dwceqos_interrupt:ioremap error \ n");
	}
#endif
	...
	if(of_find_property(pdev->dev.of_node,"support_skb_pool",&len)){
		lp->support_skb_pool = true;
		dwc_skb_pool =  skb_pool_create(&pdev->dev,"dwc_skb_pool",DWCEQOS_RX_BUF_SIZE,0);
		if(!IS_ERR_OR_NULL(dwc_skb_pool))
		dev_err(&pdev->dev,"Create ether mac skb pool sucess\n");
	}else
		lp->support_skb_pool = false;
...
# if 0
	...
	gpio_6208 = of_get_named_gpio(pdev->dev.of_node, "gpio_6208", 0);
	if (gpio_is_valid(gpio_6208)) {
		ret = gpio_request_one(gpio_6208, (GPIOF_EXPORT | GPIOF_DIR_IN), "gpio_6208");
		if (ret == 0) {
			for(i=0; i<5; i++)
			{
				val = gpio_get_value(gpio_6208);
				if(val)
					break;
				mdelay(1000);
				pr_info("6208 clk read %d\n", i+1);
			}
		} else {
...
#ifdef HOLD_ARP_FOR_EUHT
	add_timer_on(&arp_delay_timer, 1);
	mod_timer(&arp_delay_timer, jiffies + 60 * HZ);
#endif
