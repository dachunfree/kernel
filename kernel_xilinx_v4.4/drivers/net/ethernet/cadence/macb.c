...
static void gem_rx_refill(struct macb *bp)
{
	...
	while (CIRC_SPACE(bp->rx_prepared_head, bp->rx_tail, RX_RING_SIZE) > 0) {
		...
		if (bp->rx_skbuff[entry] == NULL) {
			...
			skb = netdev_alloc_skb(bp->dev, bp->rx_buffer_size+16);
			 ...
			 unsigned int last_frag)
{
	 ...
	 */
	skb = netdev_alloc_skb(bp->dev, len + NET_IP_ALIGN+16);
	if (!skb) {
		bp->stats.rx_dropped++;
...
#if 0
	/* send pause frame when receive overflow*/
	if (status & MACB_BIT(OVR) && \
			(macb_readl(bp, NCFGR) & MACB_BIT(FD))) {
...
static void at91ether_rx(struct net_device *dev)
{
	...
	while (lp->rx_ring[lp->rx_tail].addr & MACB_BIT(RX_USED)) {
		...
		skb = netdev_alloc_skb(dev, pktlen + 2 + 16);
		if (skb) {
			skb_reserve(skb, 2);
