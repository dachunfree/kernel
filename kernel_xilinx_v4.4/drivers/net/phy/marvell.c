...
static int m88e1512_read_status(struct phy_device *phydev)
{
	...
	int lpa;
	int status = 0;
    int oldpage = phy_read(phydev, 22);

    //pr_notice("read_status: autoneg %d, state %d, link %d\n", phydev->autoneg, phydev->state, phydev->link);
    // select copper
    phy_write(phydev, 22, 0);
    status = phy_read(phydev, MII_M1011_PHY_STATUS);
    if (status < 0) {
        phy_write(phydev, 22, oldpage);  // restore page
        return status;
    }

    if (status & MII_M1011_PHY_STATUS_LINK) {
        //pr_notice("copper link up\n");
    } else {
