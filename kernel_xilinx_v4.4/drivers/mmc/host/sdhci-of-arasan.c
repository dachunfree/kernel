...
static struct sdhci_pltfm_data sdhci_arasan_pdata = {
	...
	.quirks = SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
		        //SDHCI_QUIRK_BROKEN_CARD_DETECTION| MMC_CAP_NONREMOVABLE,
...
static int sdhci_arasan_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "input-clock", &new_rate) < 0) {
		pr_debug("input-clock property is missing, assuming 0\n");
		new_rate = 0;
	}

	if (new_rate > 0) {
		 if (clk_set_rate(clk_xin, new_rate) != 0 )
		 	pr_err("%s:set input clock err\n", __func__);
	}
